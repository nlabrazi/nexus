...
export class SpeechToTextService {
  ...
  async transcribe(audio: SpeechAudio, options: { signal?: AbortSignal; language?: string } = {}): Promise<string> {
    if (options.signal?.aborted) { throw new SpeechError('cancelled'); }
    if (!(audio.data instanceof Uint8Array) || audio.data.byteLength === 0 || !audio.fileName?.trim()) {
      throw new SpeechError('invalid_audio');
    }
    if (audio.data.byteLength > SPEECH_MAX_AUDIO_BYTES) { throw new SpeechError('audio_too_large'); }
    ...
    try {
      ...
      if (signal.aborted) { throw new SpeechError(timeout.signal.aborted ? 'timeout' : 'cancelled'); }
      if (typeof result !== 'string') { throw new SpeechError('invalid_response'); }

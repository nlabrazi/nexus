...
export class LocalSpeechToTextProvider implements SpeechToTextProvider {
  ...
  async transcribe(audio: SpeechAudio, options: TranscriptionOptions): Promise<string> {
    if (options.signal.aborted) { throw new SpeechError('cancelled'); }
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      try {
        child = execFile(pythonPath, args, {
          ...
          if (options.signal.aborted) { reject(new SpeechError('cancelled')); return; }
          if (error) {
            const code = error.code;
            reject(new SpeechError(code === 'ENOENT' || code === 'EACCES' ? 'unavailable' :
              code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER' ? 'invalid_response' : 'transcription_failed'));
            return;
          }
          try {
            ...
            if ('error' in result) {
              switch (result.error) {
                case 'not_configured': case 'invalid_audio': case 'audio_too_large':
                case 'invalid_configuration': case 'unavailable': case 'transcription_failed':
                  throw new SpeechError(result.error);
                default: throw new SpeechError('invalid_response');
              }
            }
            ...
            reject(parseError instanceof SpeechError ? parseError : new SpeechError('invalid_response'));
          }
        });
        ...
        else { child.stdin?.end(audio.data); }
      } catch {
        options.signal.removeEventListener('abort', onAbort);
        child?.kill('SIGKILL');

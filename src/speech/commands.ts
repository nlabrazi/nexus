  ...
  const command = vscode.commands.registerCommand('nexus.testSpeechToText', async () => {
    ...
    if (!vscode.workspace.isTrusted) {
      void vscode.window.showWarningMessage(
        'Nexus: Faites confiance au workspace pour lancer le moteur vocal local.'
      );
      return;
    }
    ...
    try {
      ...
      if (!uri || controller.signal.aborted) {
        return;
      }
      await vscode.window.withProgress(
        {
          location: vscode.ProgressLocation.Notification,
          title: 'Nexus : transcription locale…',
          cancellable: true,
        },
        async (_progress, token) => {
          ...
          try {
            ...
            if (stat.size > SPEECH_MAX_AUDIO_BYTES) {
            ...
            const text = await service.transcribe(
              { data, fileName: basename(uri.path) },
              {
      ...
      if (
        ...
        (error instanceof SpeechError && error.code === 'cancelled')
      ) {
        return;
      }
      const message =
        error instanceof SpeechError ? error.message : 'Impossible de lire le fichier audio.';

...
export class TelegramModels {
  ...
  async open(): Promise<void> {
    ...
    try {
      if (!this.controls) {
        throw new Error('La sélection des modèles est indisponible.');
      }
      ...
      if (!data.models.length) {
        throw new Error(`${this.agentName} ne propose aucun modèle disponible.`);
      }
      const menu: Menu = {
      ...
      if (this.menu !== menu) {
        this.close(menu, 'Choix fermé. Rouvrez /model.');
      }
    } catch (error) {
      if (generation === this.generation && this.samePeer(peer)) {
        this.cancel();
  ...
  async handleCallback(query: TelegramCallbackQuery): Promise<void> {
    ...
    if (
      ...
      query.message?.chat.id !== peer.chatId ||
      query.message.chat.type !== 'private'
    ) {
      await this.answer(query.id, 'Non autorisé.');
      return;
    }
    const match = /^model:([a-f0-9]{32}):(pick|effort|page|back|cancel):(\d+)$/.exec(
      query.data ?? ''
    );
    ...
    if (
      ...
      !menu ||
      menu.token !== match[1] ||
      !this.samePeer(menu.peer) ||
      menu.messageId !== query.message.message_id
    ) {
    ...
    if (menu.busy) {
      await this.answer(query.id, 'Choix en cours de traitement.');
      return;
    }
    const index = Number(match[3]);
    const action = match[2];
    if (action === 'cancel') {
      this.cancel('Sélection annulée.');
      await this.answer(query.id, 'Annulé.');
      return;
    }
    menu.busy = true;
    ...
    try {
      if (action === 'effort') {
        ...
        if (!menu.model || !option) {
          throw new Error('Effort invalide. Rouvrez /model.');
        }
        if (this.isBusy()) {
          throw new Error(
            `Une requête ${this.agentName} ou Git est en cours. Attendez sa fin puis rouvrez /model.`
          );
        }
        // Consume the menu before the first asynchronous mutation: first click wins.
        this.menu = undefined;
        clearTimeout(menu.timer);
        await this.controls!.select(
          { model: menu.model.model, effort: option.reasoningEffort },
          menu.data.context
        );
        this.close(
          menu,
          `✅ Modèle choisi : ${menu.model.displayName}\nRaisonnement : ${option.reasoningEffort}\nAppliqué à la prochaine demande /${this.promptCommand}.`
        );
        return;
      }
      if (action === 'pick') {
        if (
          menu.model ||
          index < menu.page * PAGE_SIZE ||
          index >= (menu.page + 1) * PAGE_SIZE ||
          !menu.data.models[index]
        ) {
        ...
        if (
          ...
          index >= Math.ceil(menu.data.models.length / PAGE_SIZE)
        ) {
  ...
  private render(menu: Menu): { text: string; keyboard: TelegramInlineKeyboard } {
    const button = (text: string, action: string, index = 0) => ({
      text: text.slice(0, 100),
      callback_data: `model:${menu.token}:${action}:${index}`,
    });
    if (menu.model) {
      ...
      return {
        ...
        keyboard: {
          inline_keyboard: [
            ...model.supportedReasoningEfforts.map((option, index) => [
              button(
                `${option.reasoningEffort}${option.reasoningEffort === model.defaultReasoningEffort ? ' · par défaut' : ''}`,
                'effort',
                index
              ),
            ]),
    ...
    const navigation = [];
    if (menu.page > 0) {
      navigation.push(button('‹ Précédents', 'page', menu.page - 1));
    }
    if (menu.page + 1 < pages) {
      navigation.push(button('Suivants ›', 'page', menu.page + 1));
    }
    const title =
      this.agentName === 'Antigravity'
    ...
    return {
      ...
      keyboard: {
        inline_keyboard: [
          ...menu.data.models
            .slice(menu.page * PAGE_SIZE, (menu.page + 1) * PAGE_SIZE)
            .map((model, index) => [
              button(
                `${model.model === menu.data.selected?.model ? '✓ ' : ''}${model.isDefault ? '☆ ' : ''}${model.displayName}`,
                'pick',
                menu.page * PAGE_SIZE + index
              ),
            ]),

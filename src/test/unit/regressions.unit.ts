...
suite('End-to-end regression scenarios with simulated transports', () => {
  test('restart uses the persisted offset and never replays duplicate commands', async (t) => {
    ...
    let calls = 0;
    const first = new TelegramService(saved, client, async () => {
      calls++;
      return 'Done';
    });
    ...
    first.stop();
    await oldPolling;
    const second = new TelegramService(saved, client, async () => {
      calls++;
      return 'Done';
    });
    ...
    client.push(
      message(40, '/codex stale'),
      message(41, '/codex next'),
      message(41, '/codex duplicate')
    );
    await flush();
  ...
  test('Telegram replacement during a turn keeps the Codex lock and suppresses the old reply', async (t) => {
    ...
    const first = new TelegramService(saved, oldClient, (prompt) =>
      codex.sendPrompt(prompt, '/project')
    );
    const oldPolling = first.start();
    first.start();
    ...
    await oldPolling;
    const client = new FakeTelegram();
    const second = new TelegramService(saved, client, (prompt) =>
      codex.sendPrompt(prompt, '/project')
    );
    const polling = second.start();
    t.after(async () => {
  ...
  for (const failure of ['missing-workspace', 'missing-codex', 'crash'] as const) {
    test(`${failure} reaches Telegram and permits an explicit retry without queued prompts`, async (t) => {
      ...
      const spawn = t.mock.method(childProcess, 'spawn', () => {
        if (broken && failure === 'missing-codex') {
          throw Object.assign(new Error('Missing'), { code: 'ENOENT' });
        }
        return child;
      });
      const codex = new CodexService(undefined, async () => {
        if (broken && failure === 'missing-workspace') {
          throw new WorkspaceError('workspace_missing', 'Le dossier du workspace est absent.');
        }
        return { root: '/project' };
      });
      const client = new FakeTelegram();
      const service = new TelegramService(context(), client, (prompt) =>
        codex.sendPrompt(prompt, '/project')
      );
      ...
      assert.match(
        ...
        failure === 'missing-workspace'
          ? /workspace est absent/
          : failure === 'missing-codex'
            ? /introuvable/
            : /perdue/
      );
  ...
  test('a late polling response after stop cannot execute commands in the restarted service', async (t) => {
    ...
    let calls = 0;
    t.mock.method(client, 'getUpdates', (offset: number, signal?: AbortSignal) =>
      ++calls === 1 ? oldResponse.promise : original(offset, signal)
    );
    let prompts = 0;
    ...
    t.after(async () => {
      service.stop();
      oldResponse.resolve({ ok: true, result: [] });

...
suite('Telegram branch commands', () => {
  test('lists and switches branches without starting Codex, reports errors and releases the lock', async (t) => {
    ...
    const service = new TelegramService(
      ...
      async (action) => {
        actions.push(action);
        if (action.type === 'switch' && action.name === 'missing') {
          throw new Error('Branche inconnue');
        }
    ...
    assert.deepEqual(actions, [
      { type: 'list' },
      { type: 'switch', name: 'missing' },
      { type: 'switch', name: 'staging' },
      { type: 'switch', name: 'origin/feature/test' },
    ]);
    assert.deepEqual(client.messages, [
      'master 🔒\nstaging',
      '❌ Branche inconnue',
      'Branche courante : staging',
      'Branche courante : origin/feature/test',
    ]);
  });
  ...
  test('checks authorization and command syntax before branch access', async (t) => {
    ...
    client.push(
      ...
      group,
      message(4, '/switch'),
      message(5, '/switch a b'),
      message(6, '/branches extra')
    );
    await flush();
    assert.equal(calls, 0);
    assert.deepEqual(client.messages, [
      'Usage : /switch <branche>',
      'Usage : /switch <branche>',
      'Usage : /branches',
    ]);
  });
  ...
  test('pending switch excludes mutations while status and stop remain responsive', async (t) => {
    ...
    client.push(
      message(1, '/switch staging'),
      message(2, '/stop'),
      message(3, '/switch other'),
      message(4, '/new'),
      message(5, '/codex hello'),
      message(6, '/status')
    );
    await flush();

...
suite('Codex model catalog and selection', () => {
  test('loads every catalog page, filters hidden entries and rejects cursor loops', async t => {
    ...
    child.receive({ id: calls('model/list')[0].id, result: { data: child.models, nextCursor: 'next' } });
    await flush();
    assert.deepEqual(calls('model/list')[1].params, { limit: 100, includeHidden: false, cursor: 'next' });
    child.receive({ id: calls('model/list')[1].id, result: { data: [
      { ...child.models[0], id: 'second', model: 'second' }, { ...child.models[0], model: 'hidden', hidden: true },
    ], nextCursor: null } });
    assert.deepEqual((await models).map(model => model.model), ['test-model', 'second']);
  ...
  test('rejects malformed model data and surfaces RPC failures without inventing models', async t => {
    ...
    child.receive({ id: calls('model/list')[1].id, error: { code: -32601, message: 'Unsupported' } });
    await rejected;
  });
  ...
  test('model choice is persisted per workspace and sent to session and turn RPCs only on explicit work', async t => {
    ...
    const preferences = new WorkspaceModelPreferences({ get: key => storage.get(key), update: async (key, value) => { storage.set(key, value); } });
    ...
    assert.equal(service.getStatus().model, undefined, 'pending choice is not an acknowledged session model');
    assert.equal(calls('turn/start').length, 0);
    await service.startSession('/project');
    assert.deepEqual((calls('thread/start')[0].params as { config: unknown }).config, { model_reasoning_effort: 'low' });
    ...
    assert.deepEqual((calls('thread/resume')[0].params as { config: unknown }).config, { model_reasoning_effort: 'low' });
  });
  ...
  test('rejects stale workspace/session menus, unavailable models and unsupported efforts', async t => {
    ...
    t.after(() => service.stop());
    const menu = await service.listModels('/project');
    await assert.rejects(service.selectModel('/other', { model: 'test-model', effort: 'low' }, menu.context), /contexte/);
    await assert.rejects(service.selectModel('/project', { model: 'missing', effort: 'low' }, menu.context), /plus disponible/);
    await assert.rejects(service.selectModel('/project', { model: 'test-model', effort: 'invented' }, menu.context), /plus disponible/);
    await service.newSession('/project');
    await assert.rejects(service.selectModel('/project', { model: 'test-model', effort: 'low' }, menu.context), /contexte/);
  ...
  test('a pending model save excludes turns, sessions, branch operations and duplicate selections', async t => {
    ...
    const service = new CodexService(undefined, undefined, undefined, { load: () => undefined, save: () => saved.promise });
    t.after(() => service.stop());
    const menu = await service.listModels('/project');
    ...
    await assert.rejects(service.withWorkspaceOperation(async () => ''), /opération Git ou Codex/);
    await assert.rejects(service.selectModel('/project', selection, menu.context), /opération Git ou Codex/);
    saved.resolve();
    await pending;
  ...
  test('a failed save preserves the previous choice and releases the lock', async t => {
    ...
    const service = new CodexService(undefined, undefined, undefined, { load: () => previous, save: async () => { throw new Error('Storage unavailable'); } });
    t.after(() => service.stop());
    const menu = await service.listModels('/project');
    await assert.rejects(service.selectModel('/project', { model: 'test-model', effort: 'low' }, menu.context), /Storage/);
  ...
  test('cannot select during a turn or apply a menu after stopping a pending catalog read', async t => {
    ...
    const menu = await service.listModels('/project');
    const turn = service.sendPrompt('hello');
    await assert.rejects(service.selectModel('/project', { model: 'test-model', effort: 'low' }, menu.context), /opération Git ou Codex/);
    await flush();
    child.complete();
    await turn;
    child.blockedMethods.add('model/list');
    const pending = service.selectModel('/project', { model: 'test-model', effort: 'low' }, menu.context);

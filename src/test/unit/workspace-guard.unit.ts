...
async function setup(t: TestContext, repository = true) {
  ...
  if (!repository) {
    ...
    t.mock.method(fsPromises, 'lstat', async (path: Parameters<typeof original>[0]) => {
      if (path === marker) {
        throw Object.assign(new Error('absent in fixture'), { code: 'ENOENT' });
      }
  ...
  const git = (cwd: string, ...args: string[]) =>
    execute(
      ...
      [
        ...
        '-c',
        `core.hooksPath=${join(temp, 'no-hooks')}`,
        '-C',
        cwd,
        ...args,
      ],
      {
        env: {
          ...
          GIT_CONFIG_GLOBAL: join(temp, 'no-config'),
          LC_ALL: 'C',
        },
      }
    );
  if (repository) {
    await git(root, 'init', '-b', 'feature/test');
  }
  const context: WorkspaceContext = {
    trusted: true,
    folders: [{ scheme: 'file', path: root }],
    dirtyDocuments: [],
...
suite('Workspace and Git preflight', () => {
  ...
  test('creates a local tracking branch from an explicit remote selection', async (t) => {
    ...
    assert.equal(
      (await git(root, 'rev-parse', '--abbrev-ref', '@{upstream}')).stdout.trim(),
      'origin/staging'
    );
  });
  ...
  test('accepts the repository root, including saved uncommitted and untracked files', async (t) => {
    ...
    assert.deepEqual(await guard.validate(root), {
      root,
      git: { directory: join(root, '.git'), branch: 'feature/test' },
    });
    assert.deepEqual(
      await readFile(join(root, '.git', 'index')),
      before,
      'preflight must not refresh or rewrite the index'
    );

...
function finish(child: FakeProcess, status = 'completed', turnId = 'turn') {
  child.receive({ method: 'turn/completed', params: { threadId: 'thread', turn: { id: turnId, status } } });
}

function item(child: FakeProcess, text: string, phase?: string, id = 'answer', turnId = 'turn') {
  child.receive({ method: 'item/completed', params: {
    threadId: 'thread', turnId, item: { id, type: 'agentMessage', text, phase },
  } });
}
...
suite('Codex process failures', () => {
  ...
  for (const event of ['process error', 'process exit', 'stdin error', 'stdout error', 'stderr error', 'stdout end', 'stop']) {
    test(`${event} ends the prompt without waiting for exit or a timeout`, async t => {
      const { child, service } = setup(t);
      const prompt = service.sendPrompt('hello', '/project');
      let reported = false;
      const failure = assert.rejects(prompt, { code: event === 'stop' ? 'stopped' : 'process_failed' })
        .then(() => { reported = true; });
      await flush();
      if (event === 'stop') { service.stop(); }
      else if (event === 'process exit') { child.emit('exit', null, 'SIGKILL'); }
      else if (event === 'process error') { child.emit('error', new Error('crash')); }
      else if (event === 'stdout end') { child.stdout.emit('end'); }
      else { child[event.split(' ')[0] as 'stdin' | 'stdout' | 'stderr'].emit('error', new Error('broken pipe')); }
      await flush();
...
suite('Codex timeout cancellation', () => {
  ...
  for (const response of ['missing', 'acknowledged', 'rejected']) {
    test(`an interrupt ${response} without confirmed completion closes the transport`, async t => {
      ...
      await flush();
      t.mock.timers.tick(120_000);
      if (response === 'acknowledged') { child.receive({ id: calls('turn/interrupt')[0].id, result: {} }); }
      if (response === 'rejected') {
        child.receive({ id: calls('turn/interrupt')[0].id, error: { code: -1, message: 'Cannot interrupt' } });
      }
      await flush();
      ...
      child.complete();
      assert.equal(service.getStatus().turn, undefined, 'late completion must not restore activity');
    });
  }
  ...
  for (const method of ['initialize', 'thread/start', 'thread/resume', 'turn/start']) {
    test(`${method} RPC timeout releases reservations without retrying`, async t => {
      ...
      const pending = method === 'thread/resume' ? service.resumeSession('/project', 'saved')
      ...
      child.receive({ id: calls(method)[0].id, result: { thread: { id: 'late', cwd: '/project' }, turn: { id: 'late' } } });
...
suite('Codex response and session errors', () => {
  for (const response of ['absent', 'commentary only', 'empty final']) {
    test(`an ${response} response is an error, with no replay`, async t => {
      ...
      await flush();
      if (response === 'commentary only') { item(child, 'Je vais vérifier.', 'commentary'); }
      if (response === 'empty final') {
        child.receive({ method: 'item/agentMessage/delta', params: {
          threadId: 'thread', turnId: 'turn', itemId: 'answer', delta: 'Partial text',
        } });
  ...
  test('completed final text takes precedence over streamed commentary and partial deltas', async t => {
    ...
    child.receive({ method: 'item/agentMessage/delta', params: {
      threadId: 'thread', turnId: 'turn', itemId: 'answer', delta: 'Partiel',
    } });
  ...
  test('final items in turn/completed are usable when item notifications are missing', async t => {
    ...
    const prompt = service.sendPrompt('hello', '/project');
    await flush();
    child.receive({ method: 'turn/completed', params: {
      threadId: 'thread', turn: { id: 'turn', status: 'completed', items: [
        { id: 'answer', type: 'agentMessage', text: 'Réponse récupérée', phase: 'final_answer' },
      ] },
    } });
  ...
  for (const status of ['failed', 'interrupted']) {
    test(`a ${status} terminal turn cannot return a partial response as success`, async t => {
      const { child, service } = setup(t);
      const prompt = service.sendPrompt('hello', '/project');
      const failure = assert.rejects(prompt, { code: status === 'failed' ? 'turn_failed' : 'interrupted' });
      await flush();
  ...
  test('a lost session on a live process becomes inactive without creating a replacement', async t => {
    ...
    await flush();
    child.receive({ id: calls('turn/start')[0].id, error: { code: -32600, message: 'thread not found: thread' } });
    await failure;
    assert.equal(service.isSessionActive(), false);
    assert.equal(service.getCurrentSessionId(), 'thread');
  ...
  test('Telegram reports a missing Codex and accepts a new instruction after the error', async t => {
    ...
    const bridge = new TelegramService(context(), telegram, prompt => service.sendPrompt(prompt, '/project'));
    const polling = bridge.start();
    t.after(async () => { bridge.stop(); await polling; });
    const send = (id: number) => telegram.push({ update_id: id, message: {
      text: '/codex hello', from: { id: 10 }, chat: { id: 20, type: 'private' },
    } });
    send(1);
    await flush();

...
const fileResponse = (fields: Record<string, unknown> = {}) =>
  new Response(
    JSON.stringify({
      ok: true,
      result: {
...
suite('Telegram voice download HTTP API', () => {
  ...
  test('accepts a streamed file at the size limit without optional metadata or Content-Length', async (t) => {
    ...
    const result = await new TelegramClient('SECRET').downloadVoice({
      ...voice,
      mime_type: undefined,
    });
  ...
  test('rejects an oversized voice before making any HTTP request', async (t) => {
    ...
    await assert.rejects(
      new TelegramClient('SECRET').downloadVoice({
        ...voice,
        file_size: TELEGRAM_VOICE_MAX_BYTES + 1,
      }),
      hasCode('too_large')
    );
  ...
  test('checks getFile size before downloading the body', async (t) => {
    const fetchMock = t.mock.method(globalThis, 'fetch', async () =>
      fileResponse({ file_size: TELEGRAM_VOICE_MAX_BYTES + 1 })
    );
  ...
  test('enforces the byte limit while streaming even when all declared sizes are smaller', async (t) => {
    ...
    t.mock.method(globalThis, 'fetch', async (url: string) =>
      url.endsWith('/getFile')
        ? fileResponse({ file_size: 1 })
        : new Response(
            new ReadableStream<Uint8Array>({
              pull(controller) {
                controller.enqueue(new Uint8Array(TELEGRAM_VOICE_MAX_BYTES / 2 + 1));
              },
              cancel: cancelled,
            }),
    ...
    await assert.rejects(
      new TelegramClient('SECRET').downloadVoice({ ...voice, file_size: 1 }),
      hasCode('too_large')
    );
    assert.equal(cancelled.mock.callCount(), 1);
  });
  ...
  for (const path of [
    ...
    test(`rejects an unavailable or unsafe file path: ${String(path)}`, async (t) => {
      const fetchMock = t.mock.method(globalThis, 'fetch', async () =>
        fileResponse({ file_path: path })
      );
      await assert.rejects(
        new TelegramClient('SECRET').downloadVoice(voice),
        hasCode('invalid_file')
      );
      assert.equal(fetchMock.mock.callCount(), 1);
    });
  }
  ...
  for (const failure of [
    ...
    test(`reports ${failure} without exposing the bot token`, async (t) => {
      t.mock.method(globalThis, 'fetch', async (url: string) => {
        ...
        if (url.endsWith('/getFile')) {
          if (failure === 'api-error') {
            return new Response(JSON.stringify({ ok: false, description: 'SECRET' }));
          }
        ...
        if (failure === 'file-http-error') {
          return new Response('SECRET', { status: 404 });
        }
      ...
      await assert.rejects(
        new TelegramClient('SECRET').downloadVoice(voice),
        hasCode(failure === 'empty-file' ? 'invalid_file' : 'download_failed')
      );
    });
  }
  ...
  test('a pre-cancelled operation makes no HTTP request', async (t) => {
    const fetchMock = t.mock.method(globalThis, 'fetch', async () => fileResponse());
    await assert.rejects(
      new TelegramClient('SECRET').downloadVoice(voice, AbortSignal.abort()),
      hasCode('cancelled')
    );
    assert.equal(fetchMock.mock.callCount(), 0);
  });
    ...
    for (const reason of ['cancel', 'timeout'] as const) {
      test(`${reason} interrupts ${phase} retrieval`, async (t) => {
        ...
        const rejected = assert.rejects(
          download,
          hasCode(reason === 'timeout' ? 'timeout' : 'cancelled')
        );
        await flush();
        if (reason === 'cancel') {
          caller.abort();

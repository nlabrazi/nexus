...
const workspace = (branch = 'feature/test') => ({ root: '/project', git: { directory: '/project/.git', branch } });
function setup(t: TestContext) {
  ...
  const storage = {
    get: () => data,
    update: async (_key: string, value: unknown) => { if (fail) { throw new Error('Disk full'); } data = value; },
  };
  const child = new FakeProcess();
  const spawn = t.mock.method(childProcess, 'spawn', () => child);
  const services: CodexService[] = [];
  const create = () => {
    const service = new CodexService(undefined, async () => workspace(branch), new WorkspaceSessionPersistence(storage));
    services.push(service);
    return service;
  };
  t.after(() => services.forEach(service => service.stop()));
  return { create, child, spawn, storage, data: () => data, setFailure: (value: boolean) => { fail = value; },
...
suite('Workspace session persistence', () => {
  ...
  test('a branch mismatch after reload blocks before spawning; explicit resume rebinds it', async t => {
    ...
    assert.equal((data() as { workspace: { git: { branch: string } } }).workspace.git.branch, 'feature/other');
  });
  ...
  test('storage errors block the prompt, release its reservation and permit retry', async t => {
    ...
    assert.equal(child.written.some(item => item.method === 'turn/start'), false);
    setFailure(false);
  ...
  test('validates stored data and isolates independent workspace stores', async () => {
    for (const value of [undefined, null, {}, { version: 2 }, { version: 1, id: 'bad id', workspace: workspace() },
      { version: 1, id: 'saved', workspace: { root: 'relative' } },
      { version: 1, id: 'saved', workspace: { root: '/project', git: { directory: 'relative', branch: 'feature' } } }]) {
      const store = new WorkspaceSessionPersistence({ get: () => value, update: async () => {} });
      assert.equal(store.load(), undefined);
    }
    let a: unknown;
    const storeA = new WorkspaceSessionPersistence({ get: () => a, update: async (_key, value) => { a = value; } });
    const storeB = new WorkspaceSessionPersistence({ get: () => undefined, update: async () => {} });
  ...
  test('stop during a save prevents the prompt but preserves the already selected session', async t => {
    ...
    t.mock.method(storage, 'update', async (key: string, value: unknown) => { await gate.promise; await original(key, value); });
    const service = create();
  ...
  test('serializes writes so the latest selection wins and recovers from failed writes', async () => {
    ...
    let data: unknown;
    let calls = 0;
    const store = new WorkspaceSessionPersistence({ get: () => data, update: async (_key, value) => {
      if (++calls === 1) { await gate.promise; throw new Error('Disk failure'); }
      data = value;
    } });

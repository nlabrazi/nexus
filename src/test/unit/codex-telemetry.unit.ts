...
suite('Codex usage telemetry', () => {
  test('captures cumulative usage without double-counting and isolates session snapshots', async (t) => {
    ...
    child.receive({
      ...
      params: {
        ...event.params,
        threadId: 'other',
        tokenUsage: { ...usage, total: { ...breakdown, totalTokens: 9999 } },
      },
    });
    assert.equal(service.getStatus().tokenUsage!.total.totalTokens, 1500);
    child.receive({
      ...
      params: {
        ...event.params,
        tokenUsage: { ...usage, total: { ...breakdown, totalTokens: -1 } },
      },
    });
    assert.equal(service.getStatus().tokenUsage!.total.totalTokens, 1500);
  ...
  test('receives usage before the turn acknowledgement and after completion', async (t) => {
    ...
    child.receive({
      ...
      params: {
        threadId: 'thread',
        turnId: 'turn',
        tokenUsage: { ...usage, total: { ...breakdown, totalTokens: 1600 } },
      },
    });
  ...
  test('captures the actual configuration returned by start/resume', async (t) => {
    ...
    await flush();
    child.receive({
      id: child.written.find((message) => message.method === 'thread/start')!.id,
      result: {
        thread: { id: 'thread', cwd: '/project' },
        model: 'server-model',
    ...
    child.receive({
      ...
      params: { threadId: 'unrelated', turnId: 'turn', toModel: 'other-model' },
    });
    assert.equal(service.getStatus().reroutedModel, undefined);
    child.receive({
      method: 'model/rerouted',
      params: { threadId: 'thread', turnId: 'turn', toModel: 'fallback-model' },
    });
  ...
  test('status never spawns Codex, refreshes quotas only when connected, and keeps newer notifications', async (t) => {
    ...
    child.receive({
      ...
      params: { rateLimits: { limitId: 'codex', primary: { usedPercent: 40 } } },
    });
    child.receive({
      id: reads[1].id,
      result: { rateLimits: { limitId: 'codex', primary: { usedPercent: 30 } } },
    });
    await Promise.all([pending, duplicate]);
    assert.equal(service.getStatus().rateLimits![0].primary!.usedPercent, 40);
    const denied = service.refreshStatus();
    child.receive({
      id: child.written.at(-1)!.id,
      error: { code: -32601, message: 'Account not supported' },
    });
    await denied;
  ...
  test('quota buckets remain distinct and account changes invalidate an in-flight snapshot', async (t) => {
    ...
    child.receive({
      method: 'account/rateLimits/updated',
      params: { rateLimits: { limitId: 'additional', primary: { usedPercent: 80 } } },
    });
    assert.deepEqual(
      service.getStatus().rateLimits!.map((limit) => limit.primary!.usedPercent),
      [10, 80]
    );
    ...
    child.receive({
      id,
      result: { rateLimits: { limitId: 'codex', primary: { usedPercent: 20 } } },
    });
    await refresh;

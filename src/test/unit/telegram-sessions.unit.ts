...
suite('Telegram session commands', () => {
  ...
  test('pending session changes reject duplicate commands and prompts while /status keeps working', async t => {
    ...
    const service = new TelegramService(context(), client,
      async () => { prompts++; return 'Unexpected'; },
      () => ({ workspaceCount: 0, codex: { processRunning: true, pendingApprovals: 0, sessionChanging: true } }),
      async () => { actions++; return selected.promise; });
    const polling = service.start();
    t.after(async () => { selected.resolve('fresh'); service.stop(); await polling; });
    client.push(message(1, '/new'), message(2, '/new'), message(3, '/resume saved'),
      message(4, '/codex hello'), message(5, '/status'));
    await flush();
  ...
  test('session changes are refused while a remote prompt is still running', async t => {
    ...
    let actions = 0;
    const service = new TelegramService(context(), client, () => completed.promise, undefined,
      async () => { actions++; return 'Unexpected'; });
    const polling = service.start();
    t.after(async () => { completed.resolve('Done'); service.stop(); await polling; });
  ...
  test('invalid arguments and unauthorized messages cannot change a session', async t => {
    const client = new FakeTelegram();
    let actions = 0;
    const service = new TelegramService(context(), client, undefined, undefined,
      async () => { actions++; return 'Unexpected'; });
    const polling = service.start();
    t.after(async () => { service.stop(); await polling; });
    ...
    client.push(wrongUser, wrongChat, group, message(4, '/new extra'), message(5, '/resume'), message(6, '/resume a b'));
    await flush();
    assert.equal(actions, 0);
    assert.deepEqual(client.messages, ['Usage : /new', 'Usage : /resume <id>', 'Usage : /resume <id>']);
  });
  ...
  test('a failed operation reports its error and releases the Telegram command lock', async t => {
    ...
    const service = new TelegramService(context(), client, undefined, undefined, async () => {
      if (++actions === 1) { throw new Error('Session not found'); }
      return 'fresh';
    });
    const polling = service.start();
    t.after(async () => { service.stop(); await polling; });

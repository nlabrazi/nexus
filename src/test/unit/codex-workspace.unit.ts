...
suite('Codex workspace safety integration', () => {
  ...
  test('all entry points validate before spawning and release their reservation after refusal', async t => {
    ...
    const { service, spawn } = setup(t, async () => {
      if (blocked) { throw new WorkspaceError('unsaved_documents', 'Save first'); }
      return identity();
    });
    for (const action of [() => service.startSession('/project'), () => service.newSession('/project'),
      () => service.resumeSession('/project', 'saved'), () => service.sendPrompt('hello', '/project')]) {
  ...
  test('a context change during session creation preserves the previous selection', async t => {
    ...
    await flush();
    branch = 'feature/two';
    child.receive({ id: calls('thread/start')[1].id, result: { thread: { id: 'other', cwd: '/project' } } });
    await failure;
  ...
  test('a newly dirty editor blocks the last check before turn/start', async t => {
    ...
    const { service, calls } = setup(t, async () => {
      if (++checks === 3) { throw new WorkspaceError('unsaved_documents', 'Save first'); }
      return identity();
    });
  ...
  test('validation holds the reservation and stopping it prevents a late spawn', async t => {
    ...
    const { service, spawn } = setup(t, async () => { await barrier; return identity(); });
  ...
  test('uses the canonical workspace for RPCs and removes Git routing from the child environment', async t => {
    ...
    const args = spawn.mock.calls[0].arguments as unknown as [string, string[], { env: NodeJS.ProcessEnv }];

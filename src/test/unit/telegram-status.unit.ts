...
suite('Telegram /status', () => {
  test('formats real token counters, context, selected model and quotas without double-counting cache or reasoning', () => {
    ...
    const status = formatTelegramStatus(
      {
        workspaceCount: 0,
        codex: {
          processRunning: true,
          ...
          modelSelection: { model: 'next-model', effort: 'low' },
          tokenUsage: {
            total: breakdown,
            last: { ...breakdown, totalTokens: 500 },
    ...
    const rendered = formatTelegramResponse(status)
  ...
  test('a late status refresh cannot send to a stopped Telegram service', async (_t) => {
    const client = new FakeTelegram();
    const snapshot = deferred<NexusStatusSnapshot>();
  ...
  test('reports no workspace/session without invoking Codex and reads fresh state each time', async (t) => {
    ...
    snapshot = {
      workspace: { name: 'Nexus', path: '/project' },
      workspaceCount: 1,
      codex: {
        processRunning: true,
        sessionId: 'thread',
        workspacePath: '/project',
        sessionBranch: 'feature/test',
        pendingApprovals: 0,
      },
    };
  ...
  test('answers during a prompt and approval, with turn ID and elapsed time', async (t) => {
    ...
    const snapshot: NexusStatusSnapshot = {
      ...
      codex: {
        ...
        sessionId: 'thread',
        workspacePath: '/project',
        turn: { id: 'turn-42', startedAt: 1000 },
        pendingApprovals: 1,
      },
    };
    const service = new TelegramService(

...
const audio = (): SpeechAudio => ({ data: Buffer.from('audio'), fileName: 'voice.oga', mimeType: 'audio/ogg' });
const hasCode = (code: SpeechErrorCode) => (error: unknown) => error instanceof SpeechError && error.code === code;
...
suite('Speech-to-text service', () => {
  test('forwards the original audio and requested language and preserves transcript contents', async t => {
    const transcribe = t.mock.fn<SpeechToTextProvider['transcribe']>(async () => '  Modifie src/app.ts.\nConserve /new comme texte.  ');
    const input = audio();
    const service = new SpeechToTextService({ transcribe });
    assert.equal(await service.transcribe(input, { language: 'fr' }), 'Modifie src/app.ts.\nConserve /new comme texte.');
  ...
  test('rejects empty and oversized audio before calling the provider', async t => {
    ...
    const service = new SpeechToTextService({ transcribe });
    await assert.rejects(service.transcribe({ ...audio(), data: new Uint8Array() }), hasCode('invalid_audio'));
    await assert.rejects(service.transcribe({ ...audio(), fileName: ' ' }), hasCode('invalid_audio'));
    await assert.rejects(service.transcribe({ ...audio(), data: new Uint8Array(SPEECH_MAX_AUDIO_BYTES + 1) }), hasCode('audio_too_large'));
  ...
  test('rejects invalid configuration before invoking a provider', async t => {
    ...
    for (const timeout of [0, -1, NaN, Infinity, 2 ** 31]) {
      assert.throws(() => new SpeechToTextService({ transcribe }, timeout), hasCode('invalid_configuration'));
    }
    await assert.rejects(new SpeechToTextService({ transcribe }).transcribe(audio(), { language: 'French' }), hasCode('invalid_configuration'));
    assert.equal(transcribe.mock.callCount(), 0);
  });

  test('rejects empty and malformed provider responses', async () => {
    await assert.rejects(new SpeechToTextService({ transcribe: async () => ' \n ' }).transcribe(audio()), hasCode('empty_transcript'));
    const malformed = { transcribe: async () => ({ text: 'wrong contract' }) } as unknown as SpeechToTextProvider;
    await assert.rejects(new SpeechToTextService(malformed).transcribe(audio()), hasCode('invalid_response'));
  });

  test('a pre-cancelled request never calls the provider', async t => {
    const transcribe = t.mock.fn(async () => 'Unexpected');
    await assert.rejects(new SpeechToTextService({ transcribe }).transcribe(audio(), { signal: AbortSignal.abort() }), hasCode('cancelled'));
    assert.equal(transcribe.mock.callCount(), 0);
  });
  ...
  test('cancellation is forwarded and returns promptly even if the provider ignores it', async t => {
    ...
    const result = new SpeechToTextService({ transcribe }).transcribe(audio(), { signal: caller.signal });
  ...
  test('provider errors remain safe and a failed request does not prevent a retry', async t => {
    const transcribe = t.mock.fn<SpeechToTextProvider['transcribe']>(async () => { throw new Error('SECRET credential and private transcript'); });
    const service = new SpeechToTextService({ transcribe });
  ...
  test('keeps a classified provider failure and handles synchronous cancellation safely', async () => {
    ...
    } }).transcribe(audio()), hasCode('unavailable'));
    const caller = new AbortController();
    const service = new SpeechToTextService({ transcribe: () => {
      caller.abort();
      throw new Error('SECRET');
    } });
    await assert.rejects(service.transcribe(audio(), { signal: caller.signal }), hasCode('cancelled'));
    await flush();
  });
});
